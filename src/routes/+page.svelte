<script lang="ts">
	...
	import { Music, Music2, Piano, Layers2, Hash, Search, ListOrdered } from 'lucide-svelte';

	// If returning home from a session, reset state
	$effect(() => {
		if (game.phase !== 'config') game.reset();
	});
</script>
...
<div class="flex min-h-dvh flex-col items-center justify-center gap-8 p-6">
	...
	<div class="flex flex-col gap-4 w-full max-w-sm">
		...
		<Button class="w-full" size="lg" variant="secondary" onclick={() => goto('/scale_degree')}>
			<ListOrdered class="size-4" />
			Scale Degrees
		</Button>
		<Button class="w-full" size="lg" variant="secondary" onclick={() => goto('/dyads')}>
			<Layers2 class="size-4" />
			Dyads
		</Button>
		<Button class="w-full" size="lg" variant="secondary" onclick={() => goto('/triads')}>
			<Music2 class="size-4" />
			Triads
		</Button>
		<Button class="w-full" size="lg" variant="secondary" onclick={() => goto('/sevenths')}>
			<Hash class="size-4" />
			7th Chords
		</Button>
